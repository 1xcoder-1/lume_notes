...
To make the real-time text synchronization work perfectly, the following key technical steps were implemented using **Liveblocks** and **Yjs**:

1.  **Room-Based Architecture**:
    Each note operates within a dedicated Liveblocks `RoomProvider`. This creates a isolated communication channel based on the unique `noteId`, grouping collaborators together automatically.

2.  **Yjs Shared Document Engine**:
...
5.  **Reactive Listeners**:
    Active listeners (`useOthers`) monitor presence updates, allowing the UI to reactively update the local state of all members whenever an admin or collaborator modifies the note's heading.
