    ...
    if (node.content && Array.isArray(node.content)) {
      ...
      switch (node.type) {
        ...
        case "taskItem":
          ...
          return `<li data-type="taskItem"><label><input type="checkbox" ${checked} disabled /><span>${childContent}</span></label></li>`;
        case "blockquote":
...
export async function exportNote(
  ...
  switch (format) {
    case "pdf":
      await exportToPdf(content, title, options.pdfOptions);
      break;
    case "markdown":
